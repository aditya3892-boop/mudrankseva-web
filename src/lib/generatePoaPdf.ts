...
async function loadNotoFont(): Promise<string | null> {
  ...
  try {
    ...
    console.error("[Noto/poa] Font fetch failed:", e);
    return null;
  }
}

export async function generatePoaPdf(input: PoaPdfInput): Promise<void> {
  const { poa, lang, principalName, agentName, scopeOfAuthority, duration, isTest = false } = input;
  const isMr = lang === "mr";
  ...
  if (fontBase64) {
    try {
      ...
      console.error("[Noto/poa] addFont failed:", e);
    }
  }
  ...
  useHelv(true);
  doc.setFontSize(13);
  doc.text("Power of Attorney", M, 34);
  ...
  useHelv(true);
  doc.setFontSize(7.5);
  doc.text("PARTIES & AUTHORITY DETAILS", M + 3, y + 5);
  y += 11;

  const rows: [string, string][] = [
    ["Principal",          principalName],
    ["Attorney / Agent",   agentName],
  ...
  if (isMr) {
    ...
    doc.setFontSize(8);
    doc.text(
      doc.splitTextToSize("Note: This Power of Attorney was drafted in Marathi. Please print for the Marathi version.", CW - 6),
      M + 3, y + 5,
    );
    y += 17;
  }
  ...
  if (isMr && !notoReady) {
    ...
    addFooter(1);
    doc.save(`mudrankseva_poa_${principalName.replace(/[^\w\s]/g, "").replace(/\s+/g, "_").toLowerCase() || "principal"}_marathi_fallback.pdf`);
    return;
  }

  const cleanText = poa.replace(/[─-╿]/g, "-");
  useBody();
  doc.setFontSize(9.5);
  tc(...INK);
  ...
  for (const line of lines) {
    if (y > BOTTOM) {
      ...
      useHelv(true);
      doc.setFontSize(8);
      doc.text(isTest ? "TEST DOCUMENT  \xB7  NOT FOR LEGAL USE" : "MUDRANKSEVA  \xB7  Power of Attorney (continued)", M, 8);
      y = 20;
      useBody();
      doc.setFontSize(9.5);
      tc(...INK);
  ...
  const safeName = principalName.replace(/[^\w\s]/g, "").replace(/\s+/g, "_").toLowerCase() || "principal";
  doc.save(`${isTest ? "TEST_" : ""}mudrankseva_poa_${safeName}.pdf`);
}
