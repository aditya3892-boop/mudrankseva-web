...
export interface GiftDeedPdfInput {
  deed: string;
  lang: string;
  donorName: string;
  doneeName: string;
...
async function loadNotoFont(): Promise<string | null> {
  ...
  try {
    ...
    console.error("[Noto/gift-deed] Font fetch failed:", e);
    return null;
  }
}

export async function generateGiftDeedPdf(input: GiftDeedPdfInput): Promise<void> {
  const { deed, lang, donorName, doneeName, relationship, propertyAddress, marketValue, isTest = false } = input;
  const isMr = lang === "mr";
  ...
  if (fontBase64) {
    try {
      ...
      console.error("[Noto/gift-deed] addFont failed:", e);
    }
  }
  ...
  const mktAmt     = parseInt(marketValue || "0", 10).toLocaleString("en-IN");
  ...
  if (isMr) {
    ...
    doc.setFontSize(8);
    doc.text(
      doc.splitTextToSize("Note: This gift deed was drafted in Marathi. Please print for the Marathi version.", CW - 6),
      M + 3, y + 5,
    );
    y += 17;
  }
  ...
  if (isMr && !notoReady) {
    ...
    doc.setFontSize(10);
    tc(...OXBLOOD);
    ["Marathi text rendering requires the Noto Sans Devanagari font.", "The font could not be downloaded at this time.", "",
      ...
      .forEach(line => { doc.text(line, M, y); y += 7; });
    addFooter(1);
    doc.save(`mudrankseva_gift_deed_${(donorName).replace(/[^\w\s]/g, "").replace(/\s+/g, "_").toLowerCase() || "donor"}_marathi_fallback.pdf`);
    return;
  }
  ...
  for (const line of lines) {
    if (y > BOTTOM) {
      ...
      useHelv(true);
      doc.setFontSize(8);
      doc.text(isTest ? "TEST DOCUMENT  \xB7  NOT FOR LEGAL USE" : "MUDRANKSEVA  \xB7  Gift Deed (continued)", M, 8);
      y = 20;
      useBody();
      doc.setFontSize(9.5);
      tc(...INK);
  ...
  const safeName = donorName.replace(/[^\w\s]/g, "").replace(/\s+/g, "_").toLowerCase() || "donor";
  doc.save(`${isTest ? "TEST_" : ""}mudrankseva_gift_deed_${safeName}.pdf`);
}
